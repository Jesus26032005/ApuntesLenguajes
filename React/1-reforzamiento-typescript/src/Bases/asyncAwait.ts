...
async function suma(a: number, b: number) {return a + b;}
suma(5, 10).then(resultado => { console.log(resultado); });
...
const getImage_url = async (): Promise<string> => {
    const response = await fetch(`https://api.giphy.com/v1/gifs/random?api_key=${API_KEY}&tag=&rating=g`);
    if (!response.ok) {
        throw new Error("No se logro la conexión con la API");
    }
    console.log("Se logro la conexión con la API", response);
    const {data}: GiphyRandomResponde = await response.json();
    // igual puede ser as GiphyRandomResponde despues de response.json();
    return data.images.original.url;
}

const crearImagen = async () => {
    try {
        const imagenUrl = await getImage_url();
...
crearImagen();
// Es recomendable usar con try/catch para manejar errores en funciones async/awai, asi manejas mejor los errores.